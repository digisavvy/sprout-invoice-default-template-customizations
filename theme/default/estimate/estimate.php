...
<html>
	...
	<body id="estimate" <?php body_class( 'si_default_theme' ); ?>>
				...
				<div class="row intro">
					...
					<div class="row title">
						<?php if ( 'temp' === si_get_estimate_status() ) : ?>
							<h2 class="estimate-temp"><?php esc_html_e( 'Not Yet Published', 'sprout-invoices' ) ?>
						<?php elseif ( 'approved' === si_get_estimate_status() ) : ?>
							<h2 class="estimate-approved"><?php esc_html_e( 'Estimate', 'sprout-invoices' ) ?>
						<?php else : ?>
							<h2 class="estimate-other"><?php esc_html_e( 'Estimate', 'sprout-invoices' ) ?>
						<?php endif; ?>
						<?php if ( si_is_estimate_approved() ) : ?>
						<?php _e( 'Approved', 'sprout-invoices' ) ?></h2>
							<?php else : ?>
						<?php _e( 'Not Approved', 'sprout-invoices' ) ?></h2>
							<?php endif; ?>
					</div>
		...
		<section class="row" id="paybar">
			<div class="inner estimate-expire">
				<?php
				...
				<?php if ( $time_left > 0 ) :  ?>
					...
					<?php else : ?>
						<!-- <a id="sign_doc" class="button signature_button" href="https://portal.site/agreements/abt-drains-site-website-updates/">Signature Required</a> -->
						<a href="#decline" class="button signature_button decline_estimate status_change" data-status-change="decline" data-id="<?php the_ID() ?>" data-nonce="<?php esc_attr_e( wp_create_nonce( SI_Controller::NONCE ) ) ?>"><?php esc_html_e( 'Decline Estimate', 'sprout-invoices' ) ?></a>
					<?php endif ?>
